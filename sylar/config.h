...
//F from_type T to_type  默认string
...
public:
    ...
    std::string toString() override {
        try {
            ...
            return ToStr()(m_val);  //临时对象调用重载operator()
    ...
    bool fromString(const std::string& val) override {
        try {
            ...
            setValue(FromStr()(val));   //临时对象调用重载operator()
        } catch (std::exception& e) {
            SYLAR_LOG_ERROR(SYLAR_LOG_ROOT()) << "ConfigVar::toString exception"
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,
        ...
        if(it != s_datas.end()){
            ...
            if(tmp){
                ...
                << typeid(T).name() << " realType=" << it->second->getTypeName() << " " << it->second->toString();
                return nullptr;
            }
        }
